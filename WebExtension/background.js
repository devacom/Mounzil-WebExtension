/*
 * mdm WebExtension (forked from uget-chrome-wrapper ) is an extension to integrate Mounzil
...
const INIT_MOUNZIL_CONNECTION_TIMEOUT_MS = 5 * 1000;
...
let mdmNotFound = false;
let hostName = 'com.mounzil.mdmchromewrapper';
let keywords = [];
...
function getCookies(url,callback) {
    let domain = getDomain(url);// This function was one of the best functions i've ever seen, but now it's useless. I'll not delete it because i love it... I want to spread it to world using mounzil ... RIP my friend
...
//Send cookie and send data to SendTomdm function
function setCookieAndSendTomdm(message) {
    setCookies(message).then((cookie_with_message) => {
        L("Cookies set...");
        SendTomdm(cookie_with_message);
    });
}
 ...
 *
 */
function SendTomdm(data,callback){
    SendCustomMessage({
...
function SendInitMessage(){
    //Try to connect to mounzil, if failed after timeout, disable extension
    let timeOutMounzilId = setTimeout(()=>{
        mdmNotFound = true;
        L("Mounzil not found !!")
    }, INIT_MOUNZIL_CONNECTION_TIMEOUT_MS);

    SendCustomMessage({ version: VERSION}, (response)=>{
            if(response){
                L("Connection to Mounzil was successful :)");
                clearTimeout(timeOutMounzilId);
            }else{
...
//Crafter for sending message to mdm
...
if(isChrome && !isVivaldi){
    ...
    BrowserNameSpace.downloads.onDeterminingFilename.addListener( (downloadItem,suggest)=>{
        ...
        if (!interruptDownloads) { // mdm-chrome-wrapper not reachable
            suggest();
            return;
        }
...
function isBlackListed(url) {
    ...
    if (url.startsWith("blob://")) // TODO: Mounzil currently can't handle blob type
        return true;
...
async function setInterruptDownload(interrupt) {
    ...
    await chromeStorageSetter('mdm-interrupt', interrupt);
...
async function getExtensionConfig() {
    return {
        'mdm-interrupt': await ConfigGetVal('mdm-interrupt', interruptDownloads),
...
function setContextMenu(newState) {
    if(!newState){
        ...
        try{
            //Add download with mounzil to context menu
            BrowserNameSpace.contextMenus.create({
                    title: 'Download with Mounzil',
                    id: "download_with_mdm",
                    contexts: ['link']
                }
            ...
            BrowserNameSpace.contextMenus.create({
                    title: 'Download Selected links with Mounzil',
                    id: "download_links_with_mdm",
                    contexts: ['selection']
                }
                ,()=>void chrome.runtime.lastError
            );
            ...
            BrowserNameSpace.contextMenus.create({
                    title: 'Download All Links with Mounzil',
                    id: "download_all_links_with_mdm",
                    contexts: ['page']
                }
                ,()=>void chrome.runtime.lastError);
        }catch (e) {
...
BrowserNameSpace.contextMenus.onClicked.addListener(function(info, tab) {
    ...
    switch (info.menuItemId) {
        ...
        case "download_links_with_mdm":
            ...
            break;
        case "download_all_links_with_mdm":
            BrowserNameSpace.tabs.executeScript(null, { file: "/scripts/injector.js" }, ()=>{
...
async function setConfig() {
    ...
    setInterruptDownload(config['mdm-interrupt']);
